...
public class RememberMe extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  String remember_me = request.getParameter("remember_me");   // checkbox의 체크가 없으면 null값 나옴
	  
	  //System.out.println(id + "," + remember_me);
	  ...
	  if(remember_me != null) { // if(remember_me.equals("on")){ 과 동일함
	    ...
	    cookie.setMaxAge(0);                  // 쿠키삭제를 위한 0초간 쿠키 유지
	  }
	  
	  // 쿠키는 클라이언트 전달한다. (쿠키는 클라이언트가 저장한다.)
	  response.addCookie(cookie);
	

	
	  // main 화면으로 돌아가기
	 response.sendRedirect(request.getContextPath() + "/ex07_cooke/main.jsp");
